...
std::wstring WindowsMessageMap::operator()( DWORD msg,
    LPARAM lParam, WPARAM wParam ) const
{
    constexpr std::streamsize firstColWidth = 25;

    auto it = map_.find(msg);

    auto oss = std::wostringstream();
    if ( it == map_.end() ) {
        auto padss = std::wostringstream();
        padss << "Unknown message: 0x" << std::hex << msg;
        oss << std::left << std::setw(firstColWidth) << padss.str() << std::right;
    }
    else {
        auto msgStr = it->second;
        oss << std::left << std::setw(firstColWidth) << msgStr << std::right;
    }

    oss << L"   LP: 0x" << std::hex << std::setw(8) << std::setfill( L'0' ) << lParam;
    oss << L"   WP: 0x" << std::hex << std::setw(8) << std::setfill( L'0' ) << wParam << L'\n';
